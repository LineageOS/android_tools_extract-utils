...
#
# This function will create symlink entries in the
# Android.bp and associated PRODUCT_PACKAGES list in the
# product makefile for all files in the blob list which
# has SYMLINK argument.
#
function write_symlink_packages() {
        ...
        for ARG in "${ARGS[@]}"; do
            if [[ "$ARG" =~ "SYMLINK" ]]; then
                ...
                for SYMLINK in "${SYMLINKS[@]}"; do
                    SYMBASENAME=$(basename "$SYMLINK")
                    PKGNAME=${BASENAME%.*}_${SYMBASENAME%.*}_symlink${ARCH}
                    {
    ...
    write_package_definition "${SYMPACKAGES[@]}" >> "$PRODUCTMK"
}
