...
function cleanup_ending_traces() {
    ...
    do
        PRODUCTMK=$ANDROID_ROOT/$OUTDIR/$PART.mk
        LASTLINE="$(tail -n1 $PRODUCTMK | cut -d'\' -f1 | cut -d' ' -f5)"
...
function parse_file_list() {
    ...
    while read -r line; do
        ...
        # if line contains apex, apk, jar or vintf fragment, it needs to be packaged
            ...
            if [ "$3" == "true" ]; then
                PRODUCT_COPY_FILES_LIST+=($(cut -d':' -f2 <<<"$SPEC"))
            else
    ...
    if [ -n "$3" ] && [ "$3" == "true" ]; then
        IFS=$'\n' PRODUCT_COPY_FILES_LIST=($(LC_ALL=C sort <<< "${PRODUCT_COPY_FILES_LIST[*]}")); unset IFS
    fi
    PARTITIONS=($(echo "${PRODUCT_COPY_FILES_LIST[@]}" | sed 's: :\n:g' | cut -d'/' -f1 | sort | uniq | awk '{printf("%s ", $0)}'))
}
