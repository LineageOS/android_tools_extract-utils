...
function write_blueprint_packages() {
    ...
    while IFS= read -r P; do
        ...
        if [ "$CLASS" = "SHARED_LIBRARIES" ]; then
            ...
            if ! "$OBJDUMP" -a "$ANDROID_ROOT"/"$OUTDIR"/"$SRC"/"$FILE" 2>/dev/null |grep -c 'file format elf' > /dev/null; then
            ...
            if [ "$EXTENSION" != "sh" ]; then
                printf '\ttarget: {\n'
                if "$OBJDUMP" -a "$ANDROID_ROOT"/"$OUTDIR"/"$SRC"/"$FILE" |grep -c 'file format elf64' > /dev/null; then
                    printf '\t\tandroid_arm64: {\n'
                else
                    printf '\t\tandroid_arm: {\n'
                fi
