...
function parse_file_list() {
    ...
    while read -r line; do
        ...
        local ARGS=
        if [ "$SRC_FILE" != "$STRIPPED_SPEC" ]; then
            local TARGET_FILE="$(spec_target_file "$STRIPPED_SPEC")"
            local ARGS="$(spec_target_args "$STRIPPED_SPEC" "$SPEC")"
        fi
        ...
        if suffix_match_file ".apex" "$SRC_FILE" || \
             ...
             [[ "$TARGET_ENABLE_CHECKELF" == "true" && \
                ( "$STRIPPED_SPEC" == *".so" || \
                  "$STRIPPED_SPEC" == *"bin/"* || \
                  "$STRIPPED_SPEC" == *"lib/rfsa"* ) ]] || \
             [[ "$STRIPPED_SPEC" == *"etc/vintf/manifest/"* ]]; then
            IS_PRODUCT_PACKAGE=true
        fi
