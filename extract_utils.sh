...
#
# write_shared_library_targets:
#
# Procedure that generates the list of shared libraries targets.
# It processes all the shared libraries inside the list of proprietary files
# and checks for duplicates and same lib in multiple partitions.
#
function write_shared_library_targets() {
    local ALL_LIBS=()
...
function write_product_packages() {
    ...
    # Shared libraries
    write_shared_library_targets >> "$ANDROIDBP"
...
function parse_file_list() {
    ...
    while read -r line; do
        ...
        # if line contains apex, apk, jar, vintf fragment, binary or shared lib,
        # it needs to be packaged
             ...
             [[ "$SPEC" == *"etc/vintf/manifest/"* ]] || \
             ([[ "$SPEC" == *"bin/"* ]] && ! suffix_match_file ".sh" "$(src_file "$SPEC")") || \
             ([[ "$SPEC" == *"lib/"* ]] || [[ "$SPEC" == *"lib64/"* ]] && \
              suffix_match_file ".so" "$(src_file "$SPEC")"); then
            PRODUCT_PACKAGES_LIST+=("$SPEC")
            PRODUCT_PACKAGES_HASHES+=("$HASH")
            PRODUCT_PACKAGES_FIXUP_HASHES+=("$FIXUP_HASH")
        else
