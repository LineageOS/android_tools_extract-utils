...
class File:
    ...
    def set_arg(
        self,
        k: Union[FileArgs, str],
        v: Union[Literal[True], str, List[str]],
    ) -> File:
        if isinstance(k, str):
            k = FileArgs[k]
...
class FileTree:
    ...
    def __iter__(self):
        return self.__files(self.__tree)
    ...
    def __files(
        self, subtree: file_tree_dict
    ) -> Generator[Union[List[File], File], None, None]:
    ...
    def __get_prefixed(
        ...
        parts: List[str],
        filter=True,
    ) -> Optional[Union[file_tree_dict, List[File]]]:
    ...
    def get_file(self, parts: List[str]) -> File | None:
        files = self.__get_prefixed(self.__tree, parts, filter=False)
        if files is None:
            return None
...
class FileList:
    def __init__(
        ...
        # used while updating
        self.lines_or_files: List[File | str] = []
    ...
    def __add_line(self, line: str):
        if not is_valid_line(line):
            self.lines_or_files.append(line)
            return

        # Postpone adding the files to be able to sort them based on dst
        file = File(line)
        self.lines_or_files.append(file)
    ...
    def write_to_file(self, file_path: str):
        with open(file_path, 'w') as f:
            for line_or_file in self.lines_or_files:
                f.write(f'{line_or_file}')
                if isinstance(line_or_file, File):
                    f.write('\n')

    def get_file(self, file_dst: str) -> File | None:
        parts = file_dst.split('/')
        return self.files.get_file(parts)
