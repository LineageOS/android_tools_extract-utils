...
class ExtractUtils:
    ...
    def process_module_file(
        ...
        if not copy_ctx.copy_file(file, is_firmware):
            color_print(
        ...
        file_path = path.join(module.vendor_prop_path, file.dst)
    ...
    def process_module_pinned_file(
        ...
        if not restore_ctx.copy_file(file, is_firmware):
            restored = False
    ...
    def backup_module_pinned_files(
        ...
        for proprietary_file in module.proprietary_files:
            is_firmware = isinstance(proprietary_file, FirmwareProprietaryFile)
            file_list = proprietary_file.file_list
            assert file_list is not None

            vendor_path = module.vendor_prop_path
            if is_firmware:
                vendor_path = module.vendor_radio_path
    ...
    def cleanup_module(
        ...
        os.makedirs(module.vendor_prop_path)
    ...
    def write_proprietary_file_makefiles(
        self,
        proprietary_file: ProprietaryFile,
        ctx: MakefilesCtx,
        bp_out: TextIO,
    ...
    def write_firmware_proprietary_file_makefiles(
        self,
        proprietary_file: ProprietaryFile,
        ctx: MakefilesCtx,
        board_config_mk_path: TextIO,
        android_mk_path: TextIO,
    ):
        file_list = proprietary_file.file_list
        assert file_list is not None
    ...
    def write_module_makefiles(self, module: ExtractUtilsModule):
        ...
        radio_ctx = MakefilesCtx(
            module.vendor,
            module.vendor_radio_path,
            module.vendor_radio_rel_path,
            module.vendor_radio_rel_sub_path,
            *args,
        )

        android_bp_path = path.join(module.vendor_path, 'Android.bp')
        android_mk_path = path.join(module.vendor_path, 'Android.mk')
        ...
        with open(
            ...
            product_mk_path,
            'w',
        ) as product_mk_out, open(
            board_config_mk_path,
            'w',
        ) as board_mk_out, open(
            android_mk_path,
            'w',
        ) as android_mk_out:
            ...
            write_mk_soong_namespaces(ctx, product_mk_out)

            write_mk_header(ctx, board_mk_out)
            write_mk_header(ctx, android_mk_out)

            for proprietary_file in module.proprietary_files:
                is_firmware = isinstance(
                    proprietary_file, FirmwareProprietaryFile
                )

                if is_firmware:
                    self.write_firmware_proprietary_file_makefiles(
                        proprietary_file,
                        radio_ctx,
                        bp_out,
                        android_mk_out,
                    ...
                    self.write_proprietary_file_makefiles(
                        proprietary_file,
                        ctx,
                        bp_out,
                        product_mk_out,
