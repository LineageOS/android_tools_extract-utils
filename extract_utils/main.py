...
class ExtractUtils:
    ...
    def cleanup_module(
        ...
        for proprietary_file in module.proprietary_files:
            if proprietary_file.is_firmware:
                os.makedirs(module.vendor_radio_path)
                break
    ...
    def write_rro_makefiles(
        self,
        module: ExtractUtilsModule,
        product_mk_out: TextIO,
    ):
        for rro_package in module.rro_packages:
            write_rro_package(
                module.vendor_rro_path,
    ...
    def write_module_makefiles(self, module: ExtractUtilsModule):
        ...
        with open(
            ...
            board_config_mk_path,
            'w',
        ) as board_config_mk_out, open(
            android_mk_path,
            'w',
        ) as android_mk_out:
            write_bp_header(bp_out)
            write_bp_soong_namespaces(module.namespace_imports, bp_out)

            write_mk_header(product_mk_out)
            write_mk_soong_namespace(module.vendor_rel_path, product_mk_out)

            write_mk_header(board_config_mk_out)
            write_mk_header(android_mk_out)

            self.write_rro_makefiles(module, product_mk_out)
