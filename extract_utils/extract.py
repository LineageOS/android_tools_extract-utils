...
def find_alternate_partitions(
    ...
    for partition in extract_partitions:
        ...
        alternate_partition = alternate_partition_path.split('/', 1)[0]
        if (
...
def update_extract_partitions(
    ...
    ctx.extract_partitions = list(new_extract_partitions)


def find_sparse_raw_image_paths(input_path: str):
    magic = 0xED26FF3A.to_bytes(4, 'little')
    return find_files(['super'], input_path, magic)
...
def extract_zip(
    ...
    file_paths = filter_file_paths(ctx, file_paths)
...
def extract_image(source: str, ctx: ExtractCtx, dump_dir: str):
    ...
    ctx.extra_partitions += ['super']
    ctx.extra_files += ['payload.bin']
    ...
    sparse_raw_paths = find_sparse_raw_image_paths(dump_dir)
    if sparse_raw_paths:
        print_file_paths(sparse_raw_paths, 'sparse super.img')
