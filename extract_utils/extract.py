...
def find_super_img_paths(input_path: str):
    ...
    for file in os.scandir(input_path):
        if file.name == super_img_name or file.name.startswith(sparsechunk_prefix):
...
def run_in_parallel(input_cmds: parallel_input_cmds, fatal=False):
    ...
    for input, proc in input_procs:
        code = proc.wait()
        if code == 0:
            if input in success_inputs:
                raise ValueError(f'Multiple inputs processed for {input}')
            success_inputs.add(input)

    for input in inputs:
        if input not in success_inputs:
            s = f'Failed to extract {input}'
            if fatal:
                raise ValueError(s)
            else:
                print(s)
