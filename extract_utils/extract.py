...
EXTRACTED_PARTITIONS = ['system', 'odm', 'product', 'system_ext', 'vendor']
...
class ExtractCtx:
    def __init__(self, source: str | ArgsSource, keep_dump: bool):
        self.source = source
        self.keep_dump = keep_dump


def find_files_with_magic(
    input_path: str, magic: bytes, position: int = 0
) -> List[str]:
    file_paths = []
...
def find_files_with_ext(input_path: str, ext: str):
    file_paths = []
    for file in os.scandir(input_path):
        if not path.isfile(file):
            continue
...
def find_sparse_raw_image_paths(input_path: str):
    magic = bytes([0x3A, 0xFF, 0x26, 0xED])
    return find_files_with_magic(input_path, magic)


def find_erofs_paths(input_path: str):
    magic = bytes([0xE2, 0xE1, 0xF5, 0xE0])
    return find_files_with_magic(input_path, magic, 1024)


def find_ext4_paths(input_path: str):
    magic = bytes([0x53, 0xEF])
    return find_files_with_magic(input_path, magic, 1080)


def find_payload_path(input_path: str) -> Optional[str]:
    payload_paths = find_files_with_magic(input_path, b'CrAU')
    if payload_paths:
...
def find_sparse_data_image_paths(input_path: str):
    pass


def print_file_paths(file_paths: List[str], file_type: str):
    file_paths_str = ', '.join(file_paths)
    print(f'Found {file_type} files: {file_paths_str}')
...
def extract_super_img(file_path: str, output_dir: str):
...
def extract_brotli_imgs(file_paths: List[str], output_path: str):
    ...
    for file_path in file_paths:
        ...
        procs.append(
            (file_path, [brotli_path, '-d', file_path, '-o', output_file_path])
        )
...
def extract_image(ctx: ExtractCtx, dump_dir: str, work_dir: str):
    source = ctx.source
    assert isinstance(source, str)
    ...
    if source_is_file:
        ...
        shutil.unpack_archive(source, extract_dir=work_dir)
        flatten_dir(work_dir)

    payload_bin_path = find_payload_path(work_dir)
    if payload_bin_path:
        print_file_paths([payload_bin_path], 'payload.bin')
        extract_payload_bin(payload_bin_path, work_dir)
        remove_file_paths([payload_bin_path])

    sparse_raw_paths = find_sparse_raw_image_paths(work_dir)
    if sparse_raw_paths:
        ...
        sparse_raw_paths = extract_sparse_raw_imgs(sparse_raw_paths, work_dir)
        remove_file_paths(sparse_raw_paths)

    super_img_path = find_super_img_path(work_dir)
    if super_img_path:
        print_file_paths([super_img_path], 'super.img')
        extract_super_img(super_img_path, work_dir)
        remove_file_paths([super_img_path])

    brotli_paths = find_files_with_ext(work_dir, BROTLI_EXT)
    if brotli_paths:
        print_file_paths(brotli_paths, 'brotli')
        extract_brotli_imgs(brotli_paths, work_dir)
        remove_file_paths(brotli_paths)

    sparse_data_paths = find_files_with_ext(work_dir, SPARSE_DATA_EXT)
    if sparse_data_paths:
        print_file_paths(sparse_data_paths, 'sparse data')
        extract_sparse_data_imgs(sparse_data_paths, work_dir)
        remove_file_paths(sparse_data_paths)

    erofs_paths = find_erofs_paths(work_dir)
    if erofs_paths:
        print_file_paths(erofs_paths, 'EROFS')
        extract_erofs(erofs_paths, dump_dir)

    ext4_paths = find_ext4_paths(work_dir)
    if ext4_paths:
