...
def apply_incremental(
    ...
    with (
        ...
        # Incrementals update the firmware partitions too, and those are not
        # part of the extracted partitions. Deduplicate to avoid two
        # ota_extractor runs writing the same image.
        partitions = list(
            dict.fromkeys(
                ctx.extract_partitions + files_partitions(ctx.firmware_files)
            )
        )

        # ota_extractor cannot safely read and write the same directory, so
        # apply the delta with the current images as read-only input and a
        # separate output, then overlay the produced images.
        # One ota_extractor run per partition is measurably faster than a
        # single run extracting all of them, so apply the independent
        ...
        for partition, future in futures.items():
            ...
            # ota_extractor succeeds without producing anything for partitions
            # it does not know about, which is how an unchanged partition and
            # a partition of another device look like
        ...
        # Move the images only once every partition applied, moving them as
        # they are checked would leave the output dir with the partitions of
        # two versions and the incremental unapplied, which cannot be resumed
...
def apply_incremental_chain(
    ...
    for source in sources:
        ...
        # Applying an incremental twice would fail the source hash checks or,
        # worse, silently keep the previous images
        ...
        apply_incremental(source, source_input_dir, output_dir, ctx)
