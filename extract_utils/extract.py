...
class DumpDirResult:
    def __init__(
        ...
        source_dir_not_writeable=False,
    ):
        self.dump_dir = dump_dir
        self.copy_source_dir_to_dump_dir = copy_source_dir_to_dump_dir
        self.source_not_writeable = source_dir_not_writeable
...
def get_dump_dir(
    source: str,
    ctx: ExtractCtx,
) -> Generator[DumpDirResult, None, None]:
    ...
    if not ctx.keep_dump:
        ...
        with tempfile.TemporaryDirectory() as dump_dir:
            ...
            try:
                yield DumpDirResult(
                    dump_dir,
                    copy_source_dir_to_dump_dir=path.isdir(source),
                )
    ...
    if path.isdir(source):
        ...
        yield DumpDirResult(
            dump_dir,
            source_dir_not_writeable=not os.access(source, os.W_OK),
        )
        return
