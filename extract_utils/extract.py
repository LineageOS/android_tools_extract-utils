...
def is_extract_partition_file_name(
    ...
    root, _ = file_name.split('.', 1)
    return root in extract_partitions
...
def extract_image(ctx: ExtractCtx, dump_dir: str, work_dir: str):
    ...
    if payload_bin_path:
        ...
        extract_payload_bin(ctx, payload_bin_path, work_dir)
        remove_file_paths([payload_bin_path])

    sparse_raw_paths = find_sparse_raw_image_paths(['super'], work_dir)
    if sparse_raw_paths:
    ...
    if super_img_path:
        ...
        extract_super_img(ctx, super_img_path, work_dir)
    ...
    for extract_fn in ctx.extract_fns:
        processed_files = extract_fn(ctx, work_dir, dump_dir)
        remove_file_paths(processed_files)

    # Move leftover files to dump dir
    for file in os.scandir(work_dir):
        print(f'Found leftover file {file.name}')
