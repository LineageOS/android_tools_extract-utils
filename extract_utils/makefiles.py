...
def write_bin_package(ctx: MakefilesCtx, builder: BpBuilder):
    ...
    builder.cls(ModuleClass.EXECUTABLES).name().stem().owner(ctx.vendor).target(
        ctx.rel_sub_path, file, arch, deps
    ).multilib(bits).check_elf(
        enable_checkelf
    ).no_strip().prefer().relative_install_path().specific()
...
def write_libs_package(ctx: MakefilesCtx, builder: BpBuilder):
    ...
    builder.cls(ModuleClass.SHARED_LIBRARIES).name().stem().owner(
        ctx.vendor
    ).no_strip().targets(ctx.rel_sub_path, files, arches, deps).multilibs(
        bitses
    ).check_elf(enable_check_elf).relative_install_path().prefer().specific()


def write_rfsa_package(ctx: MakefilesCtx, builder: BpBuilder):
    builder.cls(ModuleClass.RFSA).name().filename().owner(ctx.vendor).src(
        ctx.rel_sub_path
    ).relative_install_path().specific()


def write_apex_package(ctx: MakefilesCtx, builder: BpBuilder):
    builder.cls(ModuleClass.APEX).name().owner(ctx.vendor).src(
        ctx.rel_sub_path
    ).filename().specific()
...
def write_app_package(ctx: MakefilesCtx, builder: BpBuilder):
    ...
    builder.cls(ModuleClass.APPS).name().owner(ctx.vendor).src(
        ctx.rel_sub_path
    ).set('overrides', file.overrides, optional=True).set(
        'required', file.required, optional=True
    ).signature().set('dex_preopt', {'enabled': False}).set(
        'privileged', file.privileged, optional=True
    ).specific()


def write_framework_package(ctx: MakefilesCtx, builder: BpBuilder):
    builder.cls(ModuleClass.JAVA_LIBRARIES).name().owner(ctx.vendor).src(
        ctx.rel_sub_path
    ).specific()
...
def write_etc_package(ctx: MakefilesCtx, builder: BpBuilder):
    ...
    builder.cls(cls).name().owner(ctx.vendor).src(ctx.rel_sub_path).set(
...
def write_symlink_package(
    ...
    BpBuilder().cls(ModuleClass.SYMLINK).partition(part).raw_name(
        package_name
    ).specific().set('installed_location', location).set(
...
def write_bp_soong_namespaces(ctx: MakefilesCtx, out: TextIO):
    BpBuilder().rule_name('soong_namespace').set(
        'imports', ctx.vendor_imports
    ).write(out)
