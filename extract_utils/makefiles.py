...
from .partitions import ALL_PARTITIONS, APEX_PARTITIONS, PREFIX_PARTITIONS_ALTERNATIVE_PARTS_MAP, RFSA_PARTITIONS
...
class MakefilesCtx:
    def __init__(self, device: str, vendor: str,
                 ...
                 lib_fixup: Optional[Callable[[str, str], str]],
...
def write_bin_package(ctx: MakefilesCtx, builder: BpBuilder):
    ...
    if deps is not None and ctx.lib_fixup is not None:
        deps = [ctx.lib_fixup(d, partition) for d in deps]
...
def write_product_copy_files(ctx: MakefilesCtx, base_file_tree: FileTree,
                             out: TextIO):
    if not base_file_tree:
        return

    out.write('PRODUCT_COPY_FILES +=')

    for target, parts_list in PREFIX_PARTITIONS_ALTERNATIVE_PARTS_MAP.items():
        target = f'$(TARGET_COPY_OUT_{target.upper()})'

        for parts in parts_list:
            file_tree = base_file_tree.filter_prefixed(parts)

            for file in file_tree:
                rel_dst = file.dst[file_tree.parts_prefix_len:]
                line = f' \\\n    {
