...
class Source(ABC):
    ...
    @abstractmethod
    def copy_firmware(self, file: File, target_dir: str) -> bool: ...
...
class AdbSource(Source):
    ...
    def copy_firmware(self, file: File, target_dir: str) -> bool:
        ...
        if FileArgs.AB in file.args:
            partition += f'_{self.__slot}'

        try:
            run_cmd(
                ['adb', 'pull', f'/dev/block/by-name/{partition}', target_dir]
            )
            return True
        except Exception:
            return False
...
class DiskSource(Source):
    ...
    def copy_firmware(self, file: File, target_dir: str) -> bool:
        return self.copy_file(file, target_dir)
...
class CopyCtx:
    ...
    def copy_file(self, file: File, is_firmware=False):
        if is_firmware:
            return self.source.copy_firmware(file, self.target_dir)
