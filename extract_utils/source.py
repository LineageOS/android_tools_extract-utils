...
from extract_utils.utils import file_path_sha256, run_cmd, urlretrieve_resume
...
def is_downloaded_source_valid(file_path: str, ctx: SourceCtx):
    if ctx.download_sha256 is None:
        return

    file_hash = file_path_sha256(file_path)
    return file_hash == ctx.download_sha256
...
def create_downloadable_source(ctx: SourceCtx, extract_ctx: ExtractCtx):
    ...
    with download_dir_context as download_dir:
        ...
        urlretrieve_resume(
            source,
            file_path,
            expected_hash=ctx.download_sha256,
