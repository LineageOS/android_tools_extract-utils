...
def create_input_dir(source: str, ctx: SourceCtx, extract_ctx: ExtractCtx):
    # An already extracted dump can be used as-is, as long as its images were
    # kept with --keep-images
    if path.isdir(source):
        yield source
        return

    # Extract the base first, stopping at its images, the incrementals are
    # applied over them. The dump of the last incremental is the one the user
    # asked for, this one is only an intermediate, unpacking its images would
    # produce files nothing ever reads.
    input_extract_ctx = copy(extract_ctx)
    input_extract_ctx.images_only = True

    with create_extractable_source(source, ctx, input_extract_ctx) as source:
        assert isinstance(source, DiskSource)

        yield source.dump_dir
