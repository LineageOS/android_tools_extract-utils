...
def write_bin_package(out, file, lib_fixup=None, **kwargs):
...
def write_libs_package(out, files, file_list=False, lib_fixup=None):
    if file_list:
        file = files[0]
    else:
        file = files
...
def write_packages_group(cls, group, package_names, out,
    ...
    for file_or_files in group:
        fn(out, file_or_files, file_list=file_list, lib_fixup=lib_fixup)
