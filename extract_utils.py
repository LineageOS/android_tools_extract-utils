...
from elf import get_file_arch_bits, get_file_deps
...
def write_sh_package(out, file, **kwargs):
    BpBuilder('sh_binary', file) \
        .name() \
        .stem() \
        .owner() \
        .src() \
...
def write_bin_package(out, file, **kwargs):
    arch, bits = get_file_arch_bits(file)

    if file.ext == '.sh' or arch is None:
        return write_sh_package(out, file)

    deps = None
    if file.gen_deps:
        # TODO: call lib fixups
        deps = get_file_deps(file)

    BpBuilder('cc_prebuilt_binary', file) \
        .name() \
        .stem() \
        .owner() \
...
def write_libs_package(out, files, file_list=False):
    if file_list:
        file = files[0]
    else:
        file = files
    ...
    arches, bits = zip(*[get_file_arch_bits(f) for f in files])

    # TODO: add arg for different dependencies across arches
    deps = None
    if file.gen_deps:
        deps = get_file_deps(files[0])

    BpBuilder('cc_prebuilt_library_shared', file) \
        .name() \
        .stem() \
        .owner() \
        .no_strip() \
...
def write_rfsa_package(out, file, **kwargs):
    BpBuilder('prebuilt_rfsa', file) \
        .name() \
        .filename() \
        .owner() \
        .src() \
...
def write_apex_package(out, file, **kwargs):
    BpBuilder('prebuilt_apex', file) \
        .name() \
        .owner() \
        .src() \
        .filename() \
...
def write_app_package(out, file, **kwargs):
    # TODO: remove required entries from package_names
    # TODO: check if manually specified certificates are needed
    BpBuilder('android_app_import', file) \
        .name() \
        .owner() \
        .apk() \
        .signature() \
...
def write_framework_package(out, file, **kwargs):
    BpBuilder('dex_import', file) \
        .name() \
        .owner() \
        .jars() \
        .specific() \
...
def write_etc_package(out, file, **kwargs):
    if file.ext == '.xml':
        rule_name = 'prebuilt_etc_xml'
    else:
        rule_name = 'prebuilt_etc'

    BpBuilder(rule_name, file) \
        .name() \
        .owner() \
        .src() \
        .filename_from_src() \
...
def write_packages_group(cls, group, package_names, out, file_list=False):
    ...
    for file_or_files in group:
        fn(out, file_or_files, file_list=file_list)
...
def write_product_packages(files, bp_out, mk_out):
    package_names = []
...
def write_symlink_package(file, arch, symlink, package_names, out):
    ...
    BpBuilder('install_symlink') \
        ...
        .specific_raw(part) \
        .installed_location(without_prefix) \
