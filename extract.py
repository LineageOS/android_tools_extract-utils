...
if __name__ == '__main__':
    ...
    if (
        args.extract_star_firmware is not None
        and not args.extract_star_firmware
    ):
        args.extract_star_firmware = [star_firmware_regex]

    if (
        args.extract_pixel_factory is not None
        and not args.extract_pixel_factory
    ):
        args.extract_pixel_factory = [pixel_factory_image_regex]

    if (
        args.extract_pixel_firmware is not None
        and not args.extract_pixel_firmware
    ):
